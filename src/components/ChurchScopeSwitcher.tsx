...
function optionLabel(hat: RoleScope, adminLabel: string, leaderLabel: string): string {
  return `${hat.name} · ${cap(hat.level)} · ${hat.source === 'admin' ? adminLabel : leaderLabel}`
}
...
export function useCurrentScopeLabel(): string {
  ...
  if (!focusedHat) return t('nav.allRoles')
